 ...
 */
@Lazy
@Component("societyTrainingItemCheckFlowStrategy")
public class SocietyTrainingItemCheckFlowStrategy extends AbstractTrainingItemCheckFlowStrategy {
    ...
    protected BaseInfoTab getBaseInfo(String itemId) {
        ...
        sql.append(" 	ti.linkman as `联系人`,                                     ");
        sql.append(" 	date_format(ti.put_on_record_time, '%Y-%m-%d') as `备案日期` ");
        sql.append(" FROM                                                          ");
        sql.append(" 	training_item ti                                           ");
        sql.append(" LEFT JOIN enum_const tt on tt.id = ti.flag_training_type      ");
        sql.append(" LEFT JOIN enum_const tta on tta.id = ti.flag_training_target  ");
        sql.append(" LEFT JOIN enum_const ii on ii.id = ti.flag_is_legal           ");
        sql.append(" WHERE                                                         ");
        sql.append(" 	ti.id = ?                                                  ");
        String[] serialHeader = {"项目编号", "项目名称", "培训类型", "培训对象", "培训开始时间", "培训结束时间",
                "培训人数", "培训费", "备案日期"};
