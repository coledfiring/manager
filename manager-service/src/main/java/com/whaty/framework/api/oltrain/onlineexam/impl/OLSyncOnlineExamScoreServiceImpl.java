package com.whaty.framework.api.oltrain.onlineexam.impl;

import com.whaty.constant.CommonConstant;
import com.whaty.dao.GeneralDao;
import com.whaty.framework.api.oltrain.onlineexam.OLSyncOnlineExamScoreService;
 ...
 */
@Lazy
@Service("olSyncOnlineExamScoreService")
public class OLSyncOnlineExamScoreServiceImpl implements OLSyncOnlineExamScoreService {
    ...
    public Map<String, String> doSyncOnlineExamScore(Map<String, String> requestMap) {
        ...
        sql.append(" insert into ol_student_online_exam_score_history (id,fk_exam_student_id,jsonData,create_date) ");
        sql.append(" values                                                                                        ");
        sql.append(" (                                                                                             ");
        ...
        sql.append(" inner join ol_class_online_exam exam on exam.paper_no = '" + batchCode + "'          ");
        sql.append(" left join ol_student_online_exam_score sc on sc.id='" + examStu + "'                 ");
        sql.append(" where                                                                                ");
        ...
        if (StringUtils.isBlank(scId)) {
            sql.delete(0, sql.length());
            sql.append(" insert into ol_student_online_exam_score (        ");
            sql.append("     id,                                           ");
        ...
        sql.delete(0, sql.length());
        sql.append(" update ol_student_online_exam_score examStu                                          ");
